...
def loadBalanceScale():
    ...
    """
    df = pd.read_csv('.\\DataSets\\balance-scale.data',
...
def loadCarEvaluation():
    ...
    """
    df = pd.read_csv('.\\DataSets\\car-evaluation\\car.data',
                     header=None, delimiter=',')
    df = df.apply(lambda x: pd.factorize(x)[0])
...
def loadHouseVotes84():
    ...
    """
    df = pd.read_csv('.\\DataSets\\house-votes-84.data',
                     header=None, delimiter=',')
    for i in range(1, 17):
        df = df[df[i] != '?']
    df = df.apply(lambda x: pd.factorize(x)[0])
...
def loadSoybean():
    ...
    """
    df = pd.read_csv('.\\DataSets\\soybean-small.data',
                     header=None, delimiter=',')
    for i in range(35):
        df = df[df[i] != '?']
    ...
    x2 = np.array(x[[1, 2, 3, 4, 6, 7, 9, 10, 11, 15, 16,
                  18, 19, 22, 24, 26, 29, 30, 31, 32, 33]])
    x1 = oneHot(x1)
    x = np.concatenate((x1, x2), axis=1)
    return np.array(x), np.array(y, dtype=object)[0]
...
def loadSpect():
    ...
    load spect dataset
    """
    df_train = pd.read_csv('.\\DataSets\\spect.train',
                           header=None, delimiter=',')
    df_test = pd.read_csv('.\\DataSets\\spect.test',
                          header=None, delimiter=',')
...
def loadTicTacToe():
    ...
    """
    df = pd.read_csv('.\\DataSets\\tic-tac-toe.data',
                     header=None, delimiter=',')
...
def loadMonks(dataname):
    ...
    df_train = pd.read_csv(
        '.\\DataSets\\monks-{}.train'.format(index), header=None, delimiter=' ')
    df_test = pd.read_csv(
        '.\\DataSets\\monks-{}.test'.format(index), header=None, delimiter=' ')
